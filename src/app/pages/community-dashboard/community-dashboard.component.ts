      ...
      this.communityService.addMember(communityId, { email: this.newMemberData.email }).subscribe({
        ...
        error: (err: any) => {
          ...
          if (err.status === 409) {
            ...
            console.log('Kullanıcı zaten üye - üye listesi yenileniyor');
            if (this.clubInfo.id) {
              this.loadCommunityMembers(this.clubInfo.id);
            }
            this.showToast('Bu kullanıcı zaten topluluk üyesi olarak kayıtlı.', 'success');
            this.memberCurrentPage = 1;
            this.closeModal();
          } else if (err.status === 400) {
            // Bad Request - Backend'den gelen özel hata mesajlarını göster
            const errorMsg = err.error?.message || err.error?.error || 'Kullanıcı eklenirken bir sorun oluştu. Lütfen bilgileri kontrol edin.';
            this.showToast(errorMsg, 'error');
          } else if (err.status === 404) {
            // Not Found - Kullanıcı bulunamadı
            this.showToast('Bu e-posta adresi ile kayıtlı bir kullanıcı bulunamadı.', 'error');
          } else {
            // Diğer hatalar
            console.error('Üye eklenirken beklenmeyen hata:', err);
            const errorMsg = err.error?.message || err.error?.error || 'Üye eklenirken bir hata oluştu. Lütfen tekrar deneyin.';
            this.showToast(errorMsg, 'error');
          }
        },
      });
